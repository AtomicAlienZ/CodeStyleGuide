...
**Индентация** - отступ "начала строки" от нулевого символа. Пример:
```php
if ($a) {
	$b = $c
}
...
**Выравнивание** - добавление дополнительных пробельных символов в ередину строки для повышения читаемости. Пример:
```php
$basicData = array(
	'name'            => null,
	'age'             => 0,
...
```php
/* индентация */$a = array(
/* - индентация - */'name'/* ----- выравнивание ---- */=>null,
/* - индентация - */'root_priveleges'/* выравнивание */=> false
/* индентация */);
```

====

Индентация кода **строго обязательна**. Это означает: любой блок (несколько команд, следующих одна за другой) должен быть индентирован на одинаковую глубину. Глубина вложенного блока отличается от родительского ровно на один уровень.
...
В любом случае, обязательно к прочтению: [php](http://www.php.net/manual/en/language.types.boolean.php), [JavaScript](http://jibbering.com/faq/notes/type-conversion/#tcBool)

====

Если инициализация/получение данных/обработка чего-либо может быть ленивой - она **должна** быть ленивой.
...
Если что-то может *не* существовать - **обязательно** проверь на существование.
...
$.post(
	...
	{parameters: 'here'},
	function (response) {
		// Check whether response has needed parameter "success" and it's true
