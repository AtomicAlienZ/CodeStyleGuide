...
Если есть возможность сохранить "контекст выборок" - он **должен** быть сохранен.

*ПЛОХО:*
```javascript
$('.js-container').find('.js-container__something').hide();
$('.js-container').find('.js-container__somethingElse').show();
```
*ХОРОШО:*
```javascript
var $containers =$('.js-container');

$containers.find('.js-container__something').hide();
$containers.find('.js-container__somethingElse').show();
```
